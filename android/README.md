...
3. 工作流依次：标签与 `versionName` 一致性 → 工具链检查 → 标签提交须已在 `main` 上 → Release 状态与附件查重 → 构建 + 8 组测试 → 打包 → 签名核验 →
...
- **不要再在本机打包后手动上传**——本机只有 debug 签名，装不上也升级不了正式版；Release 上已有同名附件时 CI 也会中止。
- **签名连续性**：仓库变量 `ANDROID_SIGNER_SHA256` 为约定证书的 SHA-256 指纹，签名对不上的 APK 会被拦下不上传
  （手机上已装旧版的无法覆盖升级，只能卸载重装并丢失本地数据）。指纹写 apksigner 输出的 64 位十六进制即可，keytool 的大写带冒号写法也兼容。
- ⚠️ **换密钥的一次性代价（2026-09-18）**：v1.1.0～v1.1.5 是协作者本机 debug 密钥签的，与现在的正式密钥不同。
  **改用正式密钥后的第一个版本，已装用户必须卸载重装**（本地改过的费率、天地图密钥、模型密钥会丢），Release 说明里务必写明这一句。
...
- **凡上传必须 8 组测试齐全**：测试文件缺失只在 `dry_run` 下告警跳过，正式发版和手动重跑上传都会中止。
- 用 `GITHUB_TOKEN` 在别的工作流里创建的 Release 不会触发本工作流（GitHub 防递归），发版须由人发布。

发布签名密钥：

- 正式发布用的是独立的发布密钥（PKCS12、RSA 4096、有效期 10000 天、alias `iproute`），**不入库**；
  仓库 Secrets 存 `ANDROID_KEYSTORE_BASE64` / `ANDROID_KEYSTORE_PASSWORD` / `ANDROID_KEY_PASSWORD`，变量存 `ANDROID_KEY_ALIAS`。
- 流水线把密钥还原到 runner 的 `RUNNER_TEMP` 临时文件，打完包在 `if: always()` 步骤删除；APK 启用 v2 + v3 签名方案
  （minSdk 26 用不到 v1；v3 为将来轮换密钥留门）。
- **GitHub Secrets 写进去读不回来，不能当备份**：密钥文件与口令的离线备份由维护者保管（本机密钥目录 + NAS/密码管理器各一份）。
  丢失即无法再发出能覆盖升级的版本，只能换包名或让所有用户卸载重装。
- 本机 `node android/build-apk.mjs` 不带这些环境变量，仍打 debug 包自测；发版签名只在流水线里做。
  没有密钥时 release 变体产出未签名包，`build-apk.mjs` 会直接报错，不会拿 debug 签名冒充正式包。
...
Release 上的 APK 为正式发布签名，本机构建的是 debug 签名（首次构建自动生成于 `~/.android/debug.keystore`）；两者签名不同，不能互相覆盖升级。可直接侧载：
从 GitHub Release 下载 `iproute-vX.Y.Z.apk`（本机自测则用 `app-debug.apk`）发到手机（钉钉/微信/数据线），
点开安装（需允许"安装未知应用"）。无 adb 依赖；如已开 USB 调试也可 `adb install iproute-vX.Y.Z.apk`。
