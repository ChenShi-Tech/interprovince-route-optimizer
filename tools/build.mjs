...
const AC_LINKS = [
  ...
  ['CQ', 'HB', '渝鄂联络线', 500, 5000, 1.6, 'DC', '背靠背（渝鄂柔直背靠背，南北通道各 2×1250MW）',
    '川渝藏与华中东四省为异步互联（S20），此处为渝鄂柔性直流背靠背（±420kV），容量 5000MW 取自断面资料；无单独核定输电价格，暂按送出省口径计价'],
  ['GX', 'GD', '两广联络线', 500, 3000, 0.9],
  ['HB', 'HN', '鄂湘联络线', 500, 3000, 1.1],
  ['HB', 'HA', '鄂豫联络线', 500, 2500, 1.0],
  ['AH', 'JS', '皖苏联络线', 500, 3000, 0.9, 'AC', '500kV',
  ...
  ['ZJ', 'FJ', '浙闽联络线', 500, 2000, 1.2, 'AC', '500kV',
  ...
  // ---- 其他省间交流 ----
  ['NM', 'HE', '蒙西—河北联络线', 500, 4000, 1.0, 'AC', '500kV',
...
const appData = {
  schema: 'iproute-app-data/v5',   // v2：CH 新增 bidir / regional / tRev；v3：CH 新增 sendFeeRev / dirNote / marginalNote，PV 新增 inLoss / exportLoss / exportKind；v4：CH 新增 pricePending（价格待核通道），新增 NIC（非省间通道台账）
  builtAt,
