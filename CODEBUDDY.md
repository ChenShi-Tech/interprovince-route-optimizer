...
node tools/baseline-check.mjs      # 算法回归基线 1739 条
...
2. **路径唯一标识用节点序列** `nodes.join('>')`，不能用边的 `from`/`to`——对向通行的边存储方向与行进方向相反，用边拼键会把 `A→B→C` 与 `C→B→A` 误判为同一条。
3. **任何用到边方向的地方都必须传实际行进方向**。区域电网费曾因读存储方向而非行进方向，在「湖北→山西」（长南荆存为山西→湖北）上错收华中 25.6 而非华北 10.8，**导致 1739 条路线中 1504 条数值出错**。
...
- **段前系数** `= 1 / Π_{j≥i}(1 − η_j/100)`——越靠送端的段承担的电量与损耗越多
...
已验证：回归基线 1739/1739；30 个省输配电价 + 70 条通道全部有来源文号；一手原件归档在 `docs/原始文件/`（24 个文件，可离线核对）。
...
| `docs/regression-baseline-v2.json` | 503 个省对 / 1739 条路线的数值基线（iOS/RN 也必须对它跑） |
