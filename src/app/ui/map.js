...
function renderMap(){
  ...
  setTimeout(()=>{
    ...
    if(mode==='svg'){
      const box=document.getElementById('map-view');
      if(box){ box.className='topo-svg'; box.style.background='transparent'; box.style.border='0'; box.style.borderRadius='10px'; box.style.overflow='hidden'; box.innerHTML=topoSVG(); attachTopoGestures(box); }
...
function tdTileProbe(el,gen){
  try{
    ...
    const failMsg=txt=>'<span style="color:#B3261E;font-weight:600">✗ '+txt+'</span>已自动切回内置拓扑图，可核对密钥后在「天地图」视图重试。';
    const img=new Image();
    img.onload=()=>{
      if(!live()) return;
      if(img.naturalWidth>=256){
        el.innerHTML='<span style="color:#0F6E56;font-weight:600">✓ 密钥有效，底图可用。</span>';
