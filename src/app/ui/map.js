...
let tkMsgGen=0;
/* tkPhase / tkMsgHtml：密钥区反馈收起后的状态承载（change: 失败提示行收起到折叠行）。
   面板收起时页面只留折叠行一行，失败/加载中的状态改由折叠行尾标呈现（tkStateSync）；
   消息正文存 tkMsgHtml——renderMap 重建 DOM 后按它复原 #tk-msg，展开面板即见完整说明，
   与尾标状态始终一致（不写入则不显示）。tkPhase: '' | 'loading' | 'fail'。 */
let tkPhase='', tkMsgHtml='';
function tkStateSync(){
  const el=document.getElementById('map-tk-state'); if(!el) return;
  el.textContent=tkPhase==='loading'?' · 加载中…':(tkPhase==='fail'?' · 加载失败':'');
  if(tkPhase) el.dataset.kind=tkPhase; else el.removeAttribute('data-kind');
}
/* 提示写入统一入口：正文进 tkMsgHtml（重绘后复原）、状态驱动折叠行尾标，两者同步，
   不再各自 el.innerHTML 直写——避免收起态尾标与面板内正文口径打架。 */
function tkWrite(el, html, phase){
  tkMsgHtml=html||''; tkPhase=phase||'';
  if(el&&el.isConnected) el.innerHTML=tkMsgHtml;
  tkStateSync();
}
...
/* 密钥输入框改为普通文本框（change: 取消掩码）：type=password 在手机端调起密码键盘、
   看不到剪贴板建议，粘贴密钥不便——取消掩码形式，明文直显；眼睛显隐切换随之移除。 */
...
/* mapTkOpen：天地图密钥面板开合（change: 密钥输入完成后自动收起）。三态——null=未操作过，
   渲染按「无密钥默认展开引导、有密钥默认收起」；非 null=用户已显式开/合或应用后自动收起，
   渲染只看本值（口径同 mapRouteDetailOpen 会话标志：不入档，renderMap 重建按值恢复）。 */
let mapTkOpen=null;
function tdApiReady(){
...
/* 触摸视图（change: grid-map-device-fixes，design D2）：topoViewMeta 由 topoSVG 每次渲染写入
   （full=全图视野 / min=最小视野[聚焦 0.5 倍与全图 1/12 取更深] / base=本次渲染的基准视野），
...
function topoSVG(){
  ...
  /* 路线包围盒先行（feat/grid-map-deep-zoom）：深层缩放的标注/线符比例系数 lblS 取决于
     「基准视野（聚焦盒，无路线时全图）/ 当前视野」，须在画任何元素（含断面垫层）前算好。 */
  let fx0=1e9,fy0=1e9,fx1=-1e9,fy1=-1e9;
  if(selR) selR.segs.forEach(sg=>{
    const ps=chanPts(sg.e,P); if(!ps) return;
  ...
  /* 深层缩放（feat/grid-map-deep-zoom）：视野进入基准以内后，标注与线符按 lblS 反向缩放，
     屏幕尺寸近似恒定——放大看到的是更大密度的范围而不是更大的字；基准视野及以上 lblS=1，
     渲染与改动前逐像素一致。放大下限同步放宽到全图 1/12（见 topoViewMeta.min）。 */
  const topoBaseW=focusBox?(focusBox.x1-focusBox.x0):W;
  const lblS=Math.min(1,shownBox.w/topoBaseW);
  const u=v=>Math.round(v*100)/100;
  ...
  if(state.mapSec){
    ...
    CH.forEach(c=>{
      ...
      if(!ps) return;
      const hot=v.hot.has(c.id);
      edges+=`<polyline points="${ptsAttr(ps)}" fill="none" style="stroke:var(--map-section)" stroke-width="${u((hot?10:5.5)*lblS)}" opacity="${hot?0.55:0.45}" stroke-linecap="round" stroke-linejoin="round"/>`;
    });
  }
  ...
  const labelBox=(text,fs,cx,cyBase)=>{
    let w=10*lblS;   // 两侧各留 5px：估宽偏小会让「实际相交但估算相离」漏检（MAP-07 实测 337px² 叠压）；深层缩放随标注同步收缩
  ...
  if(selR){
    selR.segs.forEach((sg,i)=>{
      ...
      segs+=`<polyline points="${pts}" fill="none" style="stroke:var(--map-casing)" stroke-width="${u(6*lblS)}" stroke-linecap="round" stroke-linejoin="round"/>`;
      segs+=`<polyline points="${pts}" fill="none" stroke-width="${u(3.2*lblS)}" stroke-linecap="round" stroke-linejoin="round" data-chan="${sg.e.id}" style="stroke:var(--map-route);pointer-events:stroke"/>`;
      const m1=ps[Math.floor((ps.length-1)/2)], m2=ps[Math.ceil((ps.length-1)/2)];
      const mx=(m1[0]+m2[0])/2, my=(m1[1]+m2[1])/2;
      segs+=`<circle cx="${mx.toFixed(1)}" cy="${my.toFixed(1)}" r="${u(8.5*lblS)}" style="fill:var(--map-route)"/>`
          + `<text x="${mx.toFixed(1)}" y="${my.toFixed(1)}" style="fill:var(--on-fill)" font-size="${u(10*lblS)}" font-weight="600" text-anchor="middle" dominant-baseline="central">${i+1}</text>`;
      ...
      const sPref=(i%2?17:-14)*lblS,sAlt=(i%2?-14:17)*lblS;
      segLabelCands.push([
        ...
        {name:sg.e.n,x:c05[0],yBase:c05[1]-30*lblS},
        {name:sg.e.n,x:c05[0],yBase:c05[1]+33*lblS},
      ]);
    });
  }
  // 节点
  ...
  ordered.forEach(k=>{
    ...
    const radRaw=isEnd?6:(onRoute?4.6:2.6);
    const rad=u(radRaw*lblS);
    const fill=isEnd?'--map-end':(onRoute?'--map-route':'--map-node');
    nodes+=`<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${rad}" style="fill:var(${fill});stroke:var(--map-casing)" stroke-width="${u(1.3*lblS)}"/>`;
    if(rPal&&REGION_OF[k]) nodes+=`<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${u((radRaw+2.8)*lblS)}" fill="none" style="stroke:${rPal[REGION_OF[k]]}" stroke-width="${u(1.8*lblS)}" opacity="0.95"/>`;
    // 聚焦视野下，离线路径且出画的省名不再绘制（避免跨界半截字，spec: grid-map 聚焦）
    if(focused&&!onRoute&&(x<vbx0||x>vbx1||y<vby0||y>vby1)) return;
    putText(N(k),(onRoute?10.5:9)*lblS,x,y-rad-3*lblS,`font-size="${u((onRoute?10.5:9)*lblS)}" font-weight="${onRoute?600:400}" style="fill:var(${onRoute?'--map-label-on':'--map-label'})" text-anchor="middle"`);
  });
  const seqPad=10*lblS;
  segCircles.forEach(c=>placed.push({x0:c.mx-seqPad,y0:c.my-seqPad,x1:c.mx+seqPad,y1:c.my+seqPad}));   // 序号圆占位：站名/段名不得压圆（深层缩放随 lblS 收缩）
  // 选中路线的换流站
  let sts='';
  if(selR){
    const seen=new Set();
    selR.segs.forEach(sg=>{
      [['from',sg.e.stFrom],['to',sg.e.stTo]].forEach(([side,st])=>{
        ...
        sts+=`<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${u(3.4*lblS)}" stroke-width="${u(2*lblS)}" data-st="${st}" style="fill:var(--map-casing);stroke:var(--map-end);pointer-events:all"/>`;
        // 站名：默认在点上方，冲突换下方；仍冲突则省略文字（圆点保留，点击看站名浮层）
        const stAttrs=`font-size="${u(10*lblS)}" font-weight="600" text-anchor="middle" data-st="${st}" style="fill:var(--map-end);pointer-events:all"`;
        if(!putText(ST[st].n,10*lblS,x,y-7*lblS,stAttrs)) putText(ST[st].n,10*lblS,x,y+14*lblS,stAttrs);
      });
    });
  }
  /* 段名最后放置（相对省名/站名优先级最低，但 spec 要求逐段必标——按候选序取第一个不冲突位） */
  segLabelCands.forEach(cands=>{
    const segAttrs=`font-size="${u(10*lblS)}" font-weight="600" style="fill:var(--map-label-on);stroke:var(--map-casing)" text-anchor="middle" paint-order="stroke" stroke-width="${u(3*lblS)}"`;
    for(const c of cands){ if(putText(c.name,10*lblS,c.x,c.yBase,segAttrs)) return; }
  });
  ...
  topoViewMeta={
    ...
    /* 深层缩放（feat/grid-map-deep-zoom）：下限由全图 1/6 放宽到 1/12（约 600km→300km 视野）；
       聚焦时取「聚焦盒 0.5 倍」与「全图 1/12」的更深者。配合 topoSVG 的 lblS 反向缩放，
       放大只改变可见范围密度，标注/线符屏幕尺寸近似恒定。 */
    min:focusBox
      ?{w:Math.min(W/12,(focusBox.x1-focusBox.x0)/2),h:Math.min(H/12,(focusBox.y1-focusBox.y0)/2)}
      :{w:W/12,h:H/12},
...
/* 深层缩放（feat/grid-map-deep-zoom）：topoViewSet 只改 viewBox 时，已渲染的符号/文字仍按基准
   视野的尺寸画，视觉上会跟着放大——缩放发生后按新 lblS 重画 SVG 内容（容器、手势监听不动，
   rAF 合帧；平移不改 lblS 不重画）。topoSVG 输出自带当前视野的 viewBox，无需再补 setAttribute。 */
let topoRerenderPending=0;
function topoRerender(){
  if(topoRerenderPending) return;
  topoRerenderPending=requestAnimationFrame(()=>{
    topoRerenderPending=0;
    const box=document.getElementById('map-view');
    if(!box||!topoViewMeta) return;
    const svg=box.querySelector('svg'); if(!svg) return;
    box.innerHTML=topoSVG();
  });
}
function topoViewSet(vb){
  if(!topoViewMeta) return;
  const prevW=state.mapView?state.mapView.w:topoViewMeta.base.w;
  state.mapView=topoViewClamp(vb,topoViewMeta.full,topoViewMeta.min);
  const m=state.mapView;
  if(Math.abs(m.w-prevW)>0.01){ topoRerender(); return; }   // 缩放：viewBox 随重画一并更新，避免一帧「先放大再缩小」的闪动
...
function topoViewReset(){
  ...
  state.mapView=null;
  topoRerender();   // 复位回基准视野（lblS 回 1）；原直改 viewBox 的写法会留下缩放态的符号尺寸
}
function attachTopoGestures(box){
  ...
  const rectOf=()=>(box.querySelector('svg')||svg).getBoundingClientRect();   // 深层缩放重画会换 svg 节点，每次现查
...
function renderMap(){
  ...
  if(mode==='td'){
    /* 密钥面板折叠收纳（change: 密钥输入完成后自动收起）：收起就在「天地图密钥 ?」标题行本身，
       不另起一行——标题行挂 toggleTkPanel（行首小箭头随开合旋转，同各段明细行口径；
       问号图标不参与旋转——.tk-fold 的旋转只命中直接子级箭头，问号在 <i> 内不受影响），
       输入框与三键收进 #map-tk-body。底图切换三键独占一行在前、折叠行在其下（change: 撤销同行合并）。
       默认态——无密钥展开引导填写、有密钥收起为一行；
       点「应用密钥」输入完成后自动收起（applyTk 置 mapTkOpen=false）。
       问号图标单独弹申请说明（stopPropagation 不触发开合）；preventDefault 阻断 label
       把点击转发给输入框（点标题只开合，不弹手机键盘）。
       tk-msg 收进面板内（change: 失败提示行收起到折叠行）：收起时页面不再被一行红字占位，
       状态由折叠行尾标（#map-tk-state，加载中…/加载失败）承载，展开面板看完整说明。 */
    const tkExpanded=mapTkOpen!==null?mapTkOpen:!state.tiandituKey;
    const tkState=tkPhase==='loading'?' · 加载中…':(tkPhase==='fail'?' · 加载失败':'');
    out+=`<label class="f" style="margin:2px 0 8px">
      <span id="map-tk-toggle" class="tk-fold" role="button" aria-expanded="${tkExpanded}" aria-controls="map-tk-body" onclick="event.preventDefault();toggleTkPanel(this)">${SVG_CHEV_D}天地图密钥<i aria-hidden="true" style="display:inline-flex;color:var(--ink3)" onclick="event.stopPropagation();tkHelpDlg()">${SVG_HELP}</i><span id="map-tk-state" class="tk-state"${tkPhase?` data-kind="${tkPhase}"`:''}>${tkState}</span></span>
      <div id="map-tk-body"${tkExpanded?'':' hidden'}>
      <div class="tk-wrap"><input id="i-tk" type="text" value="${esc(state.tiandituKey||'')}" placeholder="在天地图开放平台申请后粘贴到这里" autocomplete="off" spellcheck="false">
      </div>
      <!-- 三键字号/内边距/圆角对齐 .seg.small（拓扑图/腾讯地图/天地图），两行按钮高度一致；
           触屏命中区仍由 .btn.ghost::after 透明扩区机制负责。row3 与输入框的间距 ≥12px：
           按钮上扩区约上伸 11.5px，不足会压住 input（原间距来自 label.f 的 margin-bottom:11px，
           row3 移进面板后须自带，UI-HIT 审计口径）。 -->
      <div class="row3" style="margin-bottom:10px;padding-top:12px">
      ...
      <div id="tk-msg" class="note" style="margin:0 0 4px">${tkMsgHtml}</div>
      </div>
      </label>`;
      /* 密钥申请说明收进弹框：点「天地图密钥 ?」行内问号图标弹出（title 原生 tooltip
...
/* ---------- 天地图密钥面板开合（change: 密钥输入完成后自动收起） ----------
   口径同 toggleRouteDetail：只翻转 hidden 属性与 aria-expanded、同步会话标志 mapTkOpen，
   不重走 renderMap（重绘会打断天地图底图初始化，代价过重）。 */
function toggleTkPanel(btn){
  const box=document.getElementById('map-tk-body');
  if(!box) return;
  const open=box.hasAttribute('hidden');
  if(open) box.removeAttribute('hidden'); else box.setAttribute('hidden','');
  btn.setAttribute('aria-expanded',open?'true':'false');
...
/* 「各段明细」文字尾问号弹出的站点位置说明框（change: grid-map-collapse-fee-sheet 二次收敛）：
...
function applyTk(){
  ...
  if(!k){
    ...
    if(m){
      ...
      /* 常驻提示不带折叠行尾标：空输入不改动密钥（功能正常），收起后无需状态位 */
      tkWrite(m, state.tiandituKey
  ...
  if(!cooling) tdTried='';
  mapTkOpen=false;   // 输入完成自动收起：面板折为「已保存（尾号 …）」一行，反馈见折叠行尾标 + 展开面板
  renderMap();
...
function clearTk(){
  ...
  tkPhase=''; tkMsgHtml='';   // 清掉收起态尾标与面板内提示，避免残留旧结论
...
/* 密钥应用的各阶段反馈：loading/ok/empty/loadfail/drawfail。
   ...
   全部经 tkWrite 写入：正文进面板内 #tk-msg，状态进折叠行尾标（收起态唯一可见的反馈）。 */
function tkMsg(kind){
  const el=document.getElementById('tk-msg'); if(!el) return;
  const gen=++tkMsgGen;   // 新消息使一切在途异步回写失效
  if(kind==='ok'){ tkWrite(el,'','loading'); tdTileProbe(el,gen); return; }
  if(kind==='loading'){ tkWrite(el,'<span style="color:var(--ink2)">密钥已保存到本机，正在加载天地图 API…</span>','loading'); return; }
  if(kind==='empty'){ tkWrite(el,'<span style="color:var(--error-text);font-weight:600">✗ 尚未填入密钥：请先在上方粘贴天地图 tk，再点「应用密钥」。</span>',''); return; }
  if(kind==='drawfail'){ tkWrite(el,'<span style="color:var(--error-text);font-weight:600">✗ 密钥已通过校验，但底图渲染失败（天地图 API 兼容性问题），已降级为网架清单。</span>','fail'); return; }
  if(kind==='incomplete'){ tkWrite(el,'<span style="color:var(--error-text);font-weight:600">✗ 天地图 SDK 已下载，但组件注册不完整（缺 T.Label 等叠加物）。</span>请刷新页面重试；若持续出现，说明 SDK 版本有变。已降级为网架清单。','fail'); return; }
  ...
  try{
    ...
    img.onload=()=>{
      ...
      if(img.naturalWidth>=256){
        tkWrite(el,'','');         // 成功静默：不显示「密钥有效」提示，仅清空旧消息与尾标
        state.tdDegradeNote=null;   // grid-map-device-fixes D4：有效密钥成功即清除降级说明
        return;
      }
      tkWrite(el,failMsg('瓦片返回内容异常（未能解码为有效图片）。'),'fail');
      tdDegrade('key');
    };
    img.onerror=()=>{
      if(!live()) return;
      ...
      probe.then(
        ()=>{ if(!live()) return; tkWrite(el,failMsg('瓦片请求被拒（密钥无效或被风控拦截）。'),'fail'); tdDegrade('key'); },
        ()=>{ if(!live()) return; tkWrite(el,failMsg('网络不可达，瓦片加载失败。'),'fail'); tdDegrade('net'); });
    };
    tkWrite(el,'<span style="color:var(--ink2)">SDK 已加载，正在验证瓦片…</span>','loading');
    img.src=url;
  }catch(e){}
}
...
function tkFailDiagnose(el,gen){
  if(typeof location!=='undefined'&&location.protocol==='file:'){
    tkWrite(el,'<span style="color:var(--error-text);font-weight:600">✗ 天地图 API 加载失败。</span>当前为离线包（file://）环境，请依次检查：①设备网络能否访问 api.tianditu.gov.cn（关闭飞行模式/代理）；②密钥是否有效（可在天地图开放平台「我的应用」核对）；③仍失败时点上方「拓扑图」使用内置底图。已降级为网架清单。','fail');
    return;
  }
  if(!tdAttempted){ tkFailShow(el, tdFailKind, false); return; }  // 本次未发新请求：沿用上次诊断，不额外探测
  tkWrite(el,'<span style="color:var(--ink2)">✗ 天地图 API 加载失败，正在区分原因…</span>','loading');
  let probe;
...
function tkFailShow(el, kind, fresh){
  if(kind==='server'){
    tkWrite(el, (!fresh && Date.now()<tdCooldownUntil)
      ? '<span style="color:var(--error-text);font-weight:600">天地图风控拦截仍未解除。反复重试会延长封禁。</span><span style="color:var(--ink3)">已降级为网架清单。</span>'
      : '<span style="color:var(--error-text);font-weight:600">天地图服务端拦截了本次请求/临时风控，请勿反复点击。</span><span style="color:var(--ink3)">已降级为网架清单。</span>','fail');
    return;
  }
  tkWrite(el,'<span style="color:var(--error-text);font-weight:600">✗ 本机网络到不了 api.tianditu.gov.cn。</span>若开了代理，请将其设为直连或暂时关闭。已降级为网架清单。','fail');
}
