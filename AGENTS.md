...
3. **大改动完成后同步 Slack**：功能落地、数据/费率更新、发版、重要修复、阶段收尾等大改动**完成且验证通过后**，向 Slack 频道 **#跨省电力小程序**（channel_id `C0C23DYR0S0`，私有频道，搜索不到，用 ID 直发）发一条通知——**消息第一行带用户名前缀**（如 `【daiding】`；用户名优先问用户，确认不到取 `git config user.name`），**list 列表、简洁，每条一句话说清干了什么**，中文，不等用户催。
